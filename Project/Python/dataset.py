...
class Dataset:
    ...
    def getDemand(self):
        priceArea_data = self.loadDataset(
            self.dataset_demand_name, "PriceArea")
        TotalLoad_data = self.loadDataset(
            self.dataset_demand_name, "TotalLoad")
    ...
    def scalePower(self, powerList, scalePower):
        #powerList_scaled = np.divide(powerList,self.nominal_power)*12
        
        #powerList_scaled = np.multiply(powerList, scalePower/self.nominal_power)
        
        powerList_scaled = np.divide(powerList, self.nominal_power)*scalePower
        return powerList_scaled

    def scalePowerAndDemand_old(self, demandList, powerList):
    ...
    def getPower(self, year_start, year_forward):
        ...
        # List of all hours "Power" spans
        hours_power_list = list(range(0, total_power_hours, 1))
        year_start_mapped = year_start-1980
        hours_out = hours_power_list[year_start_mapped *
                                     HOURSINYEAR:year_start_mapped*HOURSINYEAR+year_forward*HOURSINYEAR]
        self.power = []
        for i in hours_out:
